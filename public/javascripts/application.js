...
var ReportChart = function(canvas, data, multiplier) {
  var self = this;
  ...
  var start = new Date(canvas.attr("data-start"));
  ...
  var y_multiplier = parseFloat(multiplier.html());
  $("tr", data).each(function(index, row) {
    var x = parseInt($($("td", row)[0]).html())*10;
    var y = 200-(parseFloat($($("td", row)[2]).html()))*y_multiplier;

    if (index > 0) {
      self.drawLine(prev[0], prev[1], x, y, "#4444ff");
    }
